...
void setup() {
  ...
  delay(1000);
  Serial.print("Nou beginnen maar");
...
void loop() {
  ...
  if (currentMillis - previousMillisPIR >= intervalPIR) {
    ...
    if(sensorValue > 575){
      ...
      float dataTOF[16][8] = readTOF();
      int dataHLK[3] = readHLK();
      //calculateClosestPerson(dataTOF, dataHLK); // This function doesn't yet work
    }
    else{
  ...
  //Merge data form HLK en TOFs to determine something
  }


void initTOF() {
  Wire.begin();
  Wire.setClock(400000); // Zet I2C-clock op 400 kHz (standaard voor VL53L5CX)
...
void readHLK(){
  //Read HLK
  static float result[3] // distance, x, y
  ld2450.read();
    ...
    for (int i = 0; i < ld2450.getSensorSupportedTargetCount(); i++) {
      ...
      //Serial.println(result_target.distance);
...
void calculateClosestPerson(float dataTOF[16][8], int dataHLK[3]){}
